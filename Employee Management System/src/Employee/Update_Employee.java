...
class Update_Employee extends Add_Employee implements ActionListener
{
    ...
    Update_Employee(String idaa)
    {
        ...
        showData(idaa);   //idaa = empid whose data will be showed
    }
    ...
    void showData(String s)
    {
        try{
            ...
            if(rs.next())
            {
                ...
                t1.setText(rs.getString(1));  //1st column name
                t2.setText(rs.getString(2));  //2nd column fname
    ...
    {
        if(ae.getSource()==b && i==1)  //i=1 means if id is available then
        {
            try{
                conn con = new conn();
