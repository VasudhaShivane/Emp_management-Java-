...
class Remove_Employee implements ActionListener
{
    ...
    Remove_Employee()
    {
        ...
        l5.setLayout(null);
        ImageIcon img=new ImageIcon(ClassLoader.getSystemResource("Employee/icons/back1.jpg"));
        l5.setIcon(img);
        f.add(l5);
    ...
    public void actionPerformed(ActionEvent ae)
    {
        ...
        if(ae.getSource()==b)  //b = search
        {
            try
            {
                ...
                if(rs.next())
                {
                    String name = rs.getString(1);    //column no 1
                    String mob = rs.getString(2);     //column no 2
                    String email=rs.getString(3);     //column no 3
        ...
        if(ae.getSource()==b1)  //b1=remove
        {
            try{
                conn con = new conn();
                String str = "delete from employee where empid = '"+t.getText()+"'"; //employee id entered in textfield search
        ...
        if(ae.getSource()==b2) //b2 = cancel
        {
            f.setVisible(false);
           details d=new details();
        }
        if(ae.getSource()==b3)   //b3 = back
        {
            f.setVisible(false);
           details d=new details();
        }
