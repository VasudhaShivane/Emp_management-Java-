...
class Print_Data implements ActionListener{
    ...
    Print_Data(String empid) //(t.empid) passed in the  view_employeefile
    {
        try{
            conn con = new conn();
            String str = "select * from employee where empid ="+empid+" "; //to fetch or match the empid entered in search textfield
            ResultSet rs= con.s.executeQuery(str);

            while(rs.next())
            {              
                name = rs.getString("name"); //to put name in name field
        ...
        id9.add(id);  //id9 label for image on frame
        ...
        id1 = new JLabel("Name:");  //here name is label
        id1.setBounds(50,120,100,30);
        id1.setFont(new Font("serif",Font.BOLD,20));
        id9.add(id1);

        aid1 = new JLabel(name);   //here name is value in database
